...
namespace DotNetNuke.Modules.SocialUrlProvider 
{
    public class SocialUrlProvider : DotNetNuke.Entities.Urls.ExtensionUrlProvider
    {
        ...
        {
            if (HideGroupPagePath && GroupPagePathTabId > 0)
            {
        ...
        internal int GroupPagePathTabId {
            get
            {
                int groupPageTabId = GetSafeInt(Constants.GroupPageTabIdSettingName, -1);                
                return groupPageTabId;
            }
        }
        internal bool HideGroupPagePath {
            get {
                bool hideGroupPagePath = GetSafeBool(Constants.HideGroupPathPageSettingName, false);
                return hideGroupPagePath; 
            }
        }
